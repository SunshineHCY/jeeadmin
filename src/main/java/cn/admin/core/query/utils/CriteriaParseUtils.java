package cn.admin.core.query.utils;
...
import cn.admin.core.query.data.Condition;
import cn.admin.core.query.data.Queryable;
import cn.admin.core.query.data.Condition.Filter;
import cn.admin.core.query.data.Condition.Operator;
...
public class CriteriaParseUtils {
	...
	public static void parseSort(DetachedCriteria target, Queryable queryable) {
		cn.admin.core.query.data.Sort sort = queryable.getSort();
		if (sort != null) {
			for (cn.admin.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.admin.core.query.data.Sort.Direction.DESC) {
					target.addOrder(Order.desc(order.getProperty()));
				} else if (order.getDirection() == cn.admin.core.query.data.Sort.Direction.ASC) {
